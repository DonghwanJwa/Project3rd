...
public class IndexController {
	...
	public String home(BoardVO bvo,Model m) {
		...
		for(int i=0;i<blist.size();i++) {		
			//title값을 html에서 text로 변환
			String titleText = blist.get(i).getBo_title();
			String titleChanged = titleText.replaceAll("(?s)<[^>]*>(\\s*<[^>]*>)*"," ");
			blist.get(i).setBo_title(titleChanged);
			
			//cont값을 html에서 text로 변환
		...
		/* 추천 작품 */
		List<BoardVO> bolist = this.bookService.recomBook();
		for(int i=0;i<bolist.size();i++) {
			String bookHtmlText = bolist.get(i).getBookVO().getBook_name();
			String bookStrippedText = bookHtmlText.replaceAll("(?s)<[^>]*>(\\s*<[^>]*>)*", " ");//태그 모두 없앰
			String bookOneSpace = bookStrippedText.replaceAll("&nbsp;","");					//&nbsp;를 모두 자바의 띄어쓰기로 바꿈(단어단위로 끊어주는 기능때문에 &nbsp;없애야됨)
			bolist.get(i).getBookVO().setBook_name(bookOneSpace);
		}
